...
public class DeliveryServiceImpl implements DeliveryService {
    ...
    public PageResult<AdminDeliveryPartnerResponseDto> listForAdmin(
        ...
        try {
            List<UserCredential> deliveryCredentials = userCredentialRepository.findByUserType(UserType.DELIVERY_PARTNER);
            for (UserCredential cred : deliveryCredentials) {
                if (!deliveryPartnerRepository.existsByUserCredentialId(cred.getId())) {
                    String initialName = "Delivery Partner";
                    if (cred.getPhoneNumber() != null && !cred.getPhoneNumber().isBlank()) {
                        initialName = "Partner " + cred.getPhoneNumber();
                    }
                    deliveryPartnerRepository.save(DeliveryPartner.create(
                            cred.getId(),
                            initialName,
                            VehicleType.BIKE,
                            null));
            ...
            log.warn("Error synchronizing delivery partner credentials: {}", e.getMessage());
        }
