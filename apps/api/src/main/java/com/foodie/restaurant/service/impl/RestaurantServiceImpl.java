...
public class RestaurantServiceImpl implements RestaurantService {
    ...
    @Override
    @Transactional(readOnly = true)
    public com.foodie.restaurant.dto.response.SettlementStatementResponseDto getSettlementStatement(
            UUID ownerCredentialId,
            LocalDate dateFrom,
            LocalDate dateTo) {
        Restaurant restaurant = requireOwned(ownerCredentialId);
        ...
        List<Order> orders = orderRepository.findByRestaurantIdAndCreatedAtBetween(restaurant.getId(), from, to);
        ...
        BigDecimal totalPackagingFees = BigDecimal.ZERO;

        for (Order order : orders) {
            OrderStatus status = order.getStatus();
            if (status == OrderStatus.DELIVERED) {
                deliveredOrders++;
                if (order.getTotalAmount() != null) grossOrderValue = grossOrderValue.add(order.getTotalAmount());
                if (order.getDiscountAmount() != null) totalDiscounts = totalDiscounts.add(order.getDiscountAmount());
                if (order.getTaxAmount() != null) totalTaxes = totalTaxes.add(order.getTaxAmount());
            } else if (status == OrderStatus.CANCELLED || status == OrderStatus.REJECTED) {
                cancelledOrders++;
            }
        }

        BigDecimal commissionPct = restaurant.getCommissionPct() != null ? restaurant.getCommissionPct() : new BigDecimal("18.00");
        BigDecimal platformCommission = grossOrderValue.multiply(commissionPct)
                .divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
        BigDecimal netEarnings = grossOrderValue.subtract(platformCommission);

        return new com.foodie.restaurant.dto.response.SettlementStatementResponseDto(
                restaurant.getId(),
                restaurant.getName(),
                from,
                to,
                ...
                deliveredOrders,
                cancelledOrders,
                grossOrderValue.setScale(2, RoundingMode.HALF_UP),
                totalDiscounts.setScale(2, RoundingMode.HALF_UP),
                totalTaxes.setScale(2, RoundingMode.HALF_UP),
                totalPackagingFees.setScale(2, RoundingMode.HALF_UP),
                platformCommission.setScale(2, RoundingMode.HALF_UP),
                BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP),
                BigDecimal.ZERO.setScale(2, RoundingMode.HALF_UP),
                netEarnings.setScale(2, RoundingMode.HALF_UP)
        );
